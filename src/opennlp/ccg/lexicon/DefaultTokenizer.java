...
/**
 * DefaultTokenizer provides a default implementation of the Tokenizer
 * interface.
 *
...
public class DefaultTokenizer implements Tokenizer {
	...
	// factory for parsing durations, in format "PnYnMnDTnHnMnS", as defined in
	// XML Schema 1.0 section 3.2.6.1
	private DatatypeFactory datatypeFactory = null;

	/**
	 * Map from special token semantic classes to special token constants. The
	 * map is initialized in the constructor, where the standard constants (eg
	 * Tokenizer.DATE_CLASS and Tokenizer.DATE_VAL) are added.
	 */
	protected Map<String, String> specialTokenMap = null;

	/**
	 * A set containing semantic classes to replace words with for language
	 * models. Equality is checked with identity, for use with interned strings.
	 */
	...
	/**
	 * Returns whether the given semantic class is one to replace words with for
	 * language models. The sem class is assumed to have been interned.
	 */
	 ...
	 * whether to parse factors strictly. Recognized pitch accents may be
	 * appended to the word form with an underscore. If the strictFactors flag
	 * is true, then colons are always assumed to separate attribute-value
	 * pairs, and hyphens are always assumed to separate attributes from values,
	 * and thus colons or hyphens not used as separators must be escaped. If the
	 * strictFactors flag is false, then there must be at least one colon and at
	 * least one hyphen in the token to trigger parsing of factors, in which
	 * case colons or hyphens not used as separators must again be escaped;
	...
	public String isSpecialToken(String token) {
		if (token == null)
			return null;
		if (isDate(token))
			return Tokenizer.DATE_CLASS;
		if (isTime(token))
			return Tokenizer.TIME_CLASS;
		if (isNum(token))
			return Tokenizer.NUM_CLASS;
		if (isAmt(token))
			return Tokenizer.AMT_CLASS;
		if (isDur(token))
			return Tokenizer.DUR_CLASS;
		if (isNamedEntity(token))
			return Tokenizer.NE_CLASS;
		return null;
	}
	...
	/**
	 * Returns true iff the token is recognized as a date. The default
	 * implementation recognizes dates in the format yyyy.MM.dd, e.g.
	 * "2004.05.07", or *.MM.dd, e.g. "*.05.07", which is taken to mean the 5th
	 * of May (in the contextually appropriate year). Note that by including the
	 * "*." prefix, the format avoids being ambiguous between a date or number;
	 * that is, with this format, something like "10.01" is unambiguously a
	 * number, whereas "*.10.01" means the 1st of October.
	 */
	...
	/**
	 * Returns true iff the token is recognized as a time. The default
	 * implementation recognizes times in the 24-hour format HH:mm, e.g. "00:12"
	 * or "15:03".
	 */
	...
	/**
	 * Returns true iff the token is recognized as a number. The default
	 * implementation returns true if the token can be parsed as an integer or
	 * double, though not one in scientific notation.
	 */
	...
	/**
	 * Returns true iff the token is recognized as an amount. The default
	 * implementation only handles currency amounts. The token is recognized as
	 * an amount if it begins with a number and ends with an ISO-4217 currency
	 * code. (See http://www.xe.com/iso4217.htm.)
	 */
	...
	/**
	 * Returns true iff the token is recognized as a duration. The format is
	 * "PnYnMnDTnHnMnS", as defined in XML Schema 1.0 section 3.2.6.1. For
	 * example, "PT5H30" is 5 hours and 30 minutes.
	 */
	public boolean isDur(String token) {
		try {
			datatypeFactory.newDuration(token);
			return true;
		} catch (Exception exc) {
			return false;
		}
	...
	/**
	 * Returns true iff the token is recognized as a named entity (not listed in
	 * lexicon). The default implementation always returns false.
	 */
	 ...
	 * replacement. The default implementation returns the word's form - or
	 * semantic class, if apropos - followed by its pitch accent (if non-null)
	 * separated by an underscore, followed by any further attribute values,
	 * also separated by underscores. With the semantic class replacement
	 ...
	 * replacement. All factors are escaped. With the semantic class replacement
	 * option, the word form and stem are replaced with the semantic class,
	...
	/**
	 * Returns an encoding of the given string where the characters for
	 * ampersand, less-than, greater-than, apostrophe, quote, colon and hyphen
	 * are escaped using HTML conventions. Null is returned for the null string.
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if (output != null) {
				...
				case '"':
					...
					break;
				case ':':
					output.append("&#").append((int) ':').append(";");
					break;
				case '-':
					output.append("&#").append((int) '-').append(";");
					break;
	...
	/**
	 * Returns a decoding of the given string where the characters for
	 * ampersand, less-than, greater-than, apostrophe, quote, colon and hyphen
	 * (and any other character whose code is given numerically) are unescaped
	 * using HTML conventions. An exception is that ampersands may be left
	 * unescaped for convenience, when there is no following semicolon in the
	 * string. Null is returned for the null string and for the string "null".
	...
	public static String unescape(String s) {
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if (c == '&') {
				...
				throw new RuntimeException("Unable to unescape " + s.substring(i, endPos + 1)
						+ "at position " + i + " in: " + s);
			} else if (output != null)
				output.append(c);
		}
	...
	/**
	 * Returns one or more orthographic words for the given word's form. This
	 * method is called from within Sign.getWordsInXml as part of producing the
	 * textual output of realization. The default implementation checks the
	 * semantic class for a special token class, and if true, returns the result
	 * of expandDate, expandTime, expandNum, expandAmt, or expandNamedEntity, as
	 * appropriate, after first checking that the corresponding isDate, ...,
	...
	public List<String> expandWord(Word word) {
		...
		if (sc == Tokenizer.TIME_CLASS && isTime(token))
			return expandTime(token);
		if (sc == Tokenizer.NUM_CLASS && isNum(token))
			return expandNum(token);
		if (sc == Tokenizer.AMT_CLASS && isAmt(token))
			return expandAmt(token);
		if (sc == Tokenizer.DUR_CLASS && isDur(token))
			return expandDur(token);
		if (sc == Tokenizer.NE_CLASS && isNamedEntity(token))
			return expandNamedEntity(token);
		String[] words = token.split("_");
		return Arrays.asList(words);
	}

	/**
	 * Returns one or more orthographic words for the given date token. The
	 * default implementation expands the date with EnglishExpander.expandDate,
	 * using the long option if the year is present, and the medium option if
	 * not.
	 */
	...
	/**
	 * Returns one or more orthographic words for the given time token. The
	 * default implementation expands the time using EnglishExpander.expandTime.
	 */
	...
	/**
	 * Returns one or more orthographic words for the given number token. The
	 * default implementation expands the number using
	...
	/**
	 * Returns one or more orthographic words for the given amount token. The
	 * default implementation expands the number using
	...
	/**
	 * Returns one or more orthographic words for the given duration token. The
	 * default implementation expands the number using
	 ...
	 * The default implementation just splits the token using underscore as a
	 * delimiter.
	 */
	...
	/**
	 * Test: tokenize args[0], expand each token; and optionally do
	 * parseToken(args[1],true).
	 */
